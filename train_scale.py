...
def create_scale_c3():
    """
    SCALE-C3
    Strategy: Asymmetric Pyramid — Balanced Expansion Across All Three EGRR Levers
    Stages:
      1: 32x32 (C=64, T=8)
      2: 16x16 (C=256, T=10)
      3a: 8x8 (C=224, T=10)
    ...
    return _BaseRecursiveArchitecture(
        ...
        Ts=[8, 10, 10, 10, 10, 10],
        downsamples=[False, True, True, False, True, False],
        dilations=[1, 2, 4],
        num_classes=100
    )
...
def train_and_eval(epochs=10):
    ...
    print(f"\n--- Training Candidate SCALE-C3 on {device} ---")
    
    model = create_scale_c3()
    expected = 292960
    
    total_params = sum(p.numel() for p in model.parameters() if p.requires_grad)
    print(f"Total Parameters: {total_params} (Expected: {expected})")
    ...
    for epoch in range(epochs):
        ...
        if acc_top1 > best_acc:
            ...
            torch.save(model.state_dict(), "best_model_SCALE_C3.pth")
            best_acc = acc_top1

if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Train SCALE-C3 Architecture")
